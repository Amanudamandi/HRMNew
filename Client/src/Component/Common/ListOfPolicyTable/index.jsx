...
const EmpPolicyDetailTable =()=> {
    ...
    const handleSubmit = async (e) => {
        ...
        console.log("myadd policy form data is ", formData);
         // Convert allowedLateDaysInMonth and salaryCutPercentage to numbers
    const formattedData = {
        ...formData,
        allowedLateDaysInMonth: formData.allowedLateDaysInMonth ? parseInt(formData.allowedLateDaysInMonth, 10) : null,
        salaryCutPercentage: formData.salaryCutPercentage ? parseFloat(formData.salaryCutPercentage) : null
    };

    console.log("Formatted policy data to send:", formattedData);

        // Submit the form if valid
        try {
            const response = await axios.post(`${process.env.REACT_APP_SERVER_ADDRESS}/common/add-officeTimePolicy`, formattedData, {
                headers: {
            ...
            setFormData({
                ...
                deductFromLeave: false,
                continuous: null,
                allowedLateDaysInMonth: "",  // Reset to empty string
                salaryCutPercentage: ""  // Reset to empty string
            });

            setErrors({}); // Clear validation errors if any
            } else {
