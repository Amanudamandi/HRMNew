...
const ShiftDetailsTable = () => {
  ...
  const [shiftId, setShiftId] = useState(null); // Store shiftId separately
  
  // Fetch data from cookies and set to formData state
    useEffect(() => {
      const shiftData = Cookies.get("selectedShift");
      console.log("my cookies set data is ", shiftData);
      if (shiftData) {
        try {
          ...
          if (parsedData.id) {
            setShiftId(parsedData.id); // Store shiftId separately
          }
        } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const updatedFormData = { ...formData, shiftId };

   console.log('Form Data:', updatedFormData);

    try {
      const response = await axios.put(`${process.env.REACT_APP_SERVER_ADDRESS}/common/update-shift`, updatedFormData);
