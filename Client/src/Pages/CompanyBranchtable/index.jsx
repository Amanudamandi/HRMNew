...
//     // Trim input values
//   const trimmedInputData = {
//     companyID: inputData.companyID?.trim(),
//     branch: inputData.branch?.trim(),
//     address: inputData.address?.trim(),
//     pin: inputData.pin?.trim(), // Trim spaces from pin code
//   };
//     console.log("my response data branch", trimmedInputData )

//     try {
//       const response = await axios.post(`${process.env.REACT_APP_SERVER_ADDRESS}/common/add-branch`, {
//         companyId: selectedCompanyID,
//         name: trimmedInputData.branch,
//         address: trimmedInputData.address,
//         pin: trimmedInputData.pin, // Using trimmed pin code
//       });
...
//     try {
//       const response = await axios.get(`${process.env.REACT_APP_SERVER_ADDRESS}/common/show-branch`);
...
//             <table className="table-auto w-full border-collapse border border-gray-400">
//               <tbody>
//                 {companynamedata?.map(({ id, name }) => (
//                   <tr key={id}>
//                     <td className="border border-gray-400 flex justify-center items-center  h-8 w-14">
//                       <input
//                         type="checkbox"
//                         checked={selectedCompanyID === id}
//                         onChange={() => handleCompanytSelect(id)}
//                       />
//                     </td>
...
function AddNewCompany() {
  ...
  const openUpdateCompanyModal = () => {
    ...
    const selectedCompany = companynamedata.find(({ id }) => id === selectedCompanyID);
    setInputData({ ...inputData, name: selectedCompany?.name || "" });
    setIsUpdateCompanyModalOpen(true);
  };
  ...
  const openUpdateBranchModal = () => {
    ...
    // Find the branch data based on the selectedBranchID
    const selectedBranch = showAllCompanyDetails.find(({ id }) => id === selectedBranchID);
    if (selectedBranch) {
      // Set the input data with the selected branch details
      setInputData({
        ...inputData,
  ...
  const fetchAllCompanyDetails = async () => {
    ...
    try {
      ...
      alert("Unable to fetch branches for the selected company", error);
    }
  };
  ...
  const updateBranchName = async (e) => {
    ...
    const finalData= {
      ...
      address: inputData.address,
      // pin: inputData.pin,
      pin: inputData.pin ? String(inputData.pin) : "",
    }

    console.log("my final update branch data", finalData)
    try {
      await axios.put(`${process.env.REACT_APP_SERVER_ADDRESS}/common/update-branch`, finalData);
      alert("Branch updated successfully");
      closeUpdateBranchModal();
  ...
  return (
    <div className="pl-2 w-full pr-2 pt-4 ">
      ...
      <div className="flex gap-10 mt">
        {/* Company Table */}
        <div className="w-[30%] max-h-[250px] overflow-y-auto relative border border-gray-400 rounded">
      ...
      <div className="w-full ">
          <table className="table-auto w-full border border-black">
            ...
            <tbody>
              {showAllCompanyDetails?.map(({ id, name, address, pin }) => (
                <tr key={id}>
                  <td className="border-t border-gray-300 flex justify-center items-center align-middle h-8">
                    <input 
                      type="checkbox"
                      checked={selectedBranchID === id}
