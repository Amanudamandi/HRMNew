...
import { toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { useAuth } from '../../Component/Context/MyContext.jsx';
axios.defaults.withCredentials = true;
...
function FrontPage() {
  ...
  const { loginAPI } = useAuth(); // Use loginAPI from context
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const success = await loginAPI(inputData, setErrors, setLoading);
    if (success) {
      toast.success("Login successfully!", { position: "top-right", autoClose: 2000 });
      setInputData({ employeeCode: "", password: "" });
      
      // navigation is handled inside loginAPI, so no need to do it again here
     navigate("/layout/dashboard");
    }
  };
      ...
      <div className="flex flex-col md:flex-row flex-grow">
        ...
        <div className="md:w-1/2 flex flex-col items-center justify-center px-6 py-8">
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-md bg-white p-6 shadow-lg rounded-lg">
            ...
            <button
                type="submit"
                className="w-full text-white py-2 rounded-lg font-semibold hover:bg-purple-600 flex items-center justify-center gap-2"
